...
class NotificationItem(QWidget):
    ...
    def __init__(self, title, message, item, ntype=0, callback=None, *args, **kwargs):
        super(NotificationItem, self).__init__(*args, **kwargs)
        self.item = item
        self.callback = callback
        layout = QHBoxLayout(self, spacing=0)
        layout.setContentsMargins(0, 0, 0, 0)
        self.bg_widget = QWidget(self)  # 背景控件, 用于支持动画效果
        layout.addWidget(self.bg_widget)

        layout = QGridLayout(self.bg_widget)
        ...
        layout.addItem(QSpacerItem(40, 20, QSizePolicy.Expanding, QSizePolicy.Minimum), 0, 2)
        layout.addWidget(self.label_close, 0, 3)
        layout.addWidget(self.label_message, 1, 1, 1, 2)
    ...
    def mousePressEvent(self, event):
        ...
        if not w:
            return
        if w == self.label_close:  # 点击关闭图标
            self.closed.emit(self.item)
        elif w == self.label_message and self.callback and callable(self.callback):
            # 点击消息内容
            self.callback()  # 回调
...
class NotificationWindow(QListWidget):
    ...
    def info(cls, title, message, callback=None):
        ...
        w = NotificationItem(title, message, item, NotificationIcon.Info, callback, cls._instance)
    ...
    def success(cls, title, message, callback=None):
        cls._create_instance()
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, NotificationIcon.Success, callback, cls._instance)
        w.closed.connect(cls._instance.remove_item)
        item.setSizeHint(QSize(cls._instance.width() - cls._instance.spacing(), w.height()))
        cls._instance.setItemWidget(item, w)
    ...
    def warning(cls, title, message, callback=None):
        cls._create_instance()
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item,  NotificationIcon.Warning, callback, cls._instance)
        w.closed.connect(cls._instance.remove_item)
        item.setSizeHint(QSize(cls._instance.width() - cls._instance.spacing(), w.height()))
        cls._instance.setItemWidget(item, w)
    ...
    def error(cls, title, message, callback=None):
        cls._create_instance()
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, NotificationIcon.Error, callback)
        w.closed.connect(cls._instance.remove_item)
